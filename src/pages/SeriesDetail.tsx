...
import { useGetSerialById } from '../apis/serielDetails';

function SeriesDetail() {
  const { id } = useParams();

  const { data: serial } = useGetSerialById(id)
  ...
  return (
    <div className="min-h-screen pb-20">
      ...
      <div className="relative h-[70vh]">
        ...
        >
          <img
            src={`https://image.tmdb.org/t/p/w500${serial?.backdrop_path}`}
            alt={serial?.title}
        ...
        <div className="absolute bottom-0 left-0 right-0">
          <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pb-8">
            ...
            >
              <h1 className="text-5xl font-bold mb-4">{serial?.original_title}</h1>
              <div className="flex items-center gap-6 text-sm mb-6">
                <div className="flex items-center gap-2">
                  <Star className="w-4 h-4 text-yellow-400" fill="currentColor" />
                  <span>{serial?.popularity}</span>
                </div>
                <div className="flex items-center gap-2">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
          ...
          >
            ...
            <p className="text-gray-300 mb-6">{serial?.overview}</p>

            <div className="grid grid-cols-2 gap-8">
              <div>
                <h3 className="font-semibold mb-2">Director</h3>
                <p className="text-gray-300">{serial?.director}</p>
              </div>
              <div>
                <h3 className="font-semibold mb-2">Genre</h3>
                <div className="flex flex-wrap gap-2">
                  {serial?.genres.map((genre:any, idx:number) => (
                    <span
                      key={idx}
